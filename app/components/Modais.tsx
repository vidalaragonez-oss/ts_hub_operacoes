  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-[#2e2c29] bg-[#111010] shrink-0 px-2">
          {(
            [
            ...
            ] as { key: "perfil" | "campanhas" | "financeiro" | "integracoes"; label: string; icon: string }[]
          ).map(tab => (
            <button
              key={tab.key}
              type="button"
              onClick={() => setActiveTab(tab.key)}
              className={`flex items-center gap-1.5 px-3 py-3 text-[10px] font-bold uppercase tracking-wider border-b-2 transition-all whitespace-nowrap ${
                activeTab === tab.key
